...
class AiohttpTransport:
    ...
    def __init__(self,
        ...
        request_kwargs['headers'] = {**self.default_headers, **request_kwargs.get('headers', {})}
    ...
    async def __call__(self,
                       ...
                       session=None,
                       request_kwargs: Optional[dict] = None) -> str:
        """
        Call method.
        You can pass any aiohttp request kwarg such as `ssl`, `timeout`, `headers`, `params` etc.
        ...
        request_kwargs = {**self.request_kwargs, 'data': request_string, **(request_kwargs or {})}
...
def create_aiohttp_client(url: str,
                          session=None,
                          request_kwargs: Optional[dict] = None,
                          ...
                          dumps: Callable[[Any], str] = jarpc.format.json_dumps) -> jarpc.AsyncJarpcClient:
    """
    ...
    """
    transport = AiohttpTransport(url=url, session=session, request_kwargs=request_kwargs)
    return jarpc.AsyncJarpcClient(transport=transport, default_ttl=default_ttl, default_rpc_ttl=default_rpc_ttl,
                                  default_notification_ttl=default_notification_ttl, loads=loads, dumps=dumps)
